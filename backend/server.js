...
// Rate Limiting (Acts as a basic WAF against spam / DDoS attacks)
...
// Strict Rate Limiting (Anti-Brute Force for Admin Login)
...
// Login endpoint, protected by anti-brute force limiter
app.post('/api/admin/login', loginLimiter, [
    // Validate inputs to prevent NoSQL injection via object payloads
    body('username').trim().isString().notEmpty().escape(),
    body('password').trim().isString().notEmpty().escape()
], (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
